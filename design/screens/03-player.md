...
Der Fokus liegt nach dem Oeffnen auf der Timeline.

## Overlay ausblenden

Das Overlay blendet nach 5 Sekunden Inaktivitaet automatisch aus.

Bei erneutem Oeffnen wird der letzte sinnvolle Fokus wiederhergestellt.

## Initialfokus

Beim Oeffnen des Overlays liegt der Fokus auf der Timeline.
...
Links/Rechts zeigen einen Hinweis, dass Timeshift fuer diesen Sender nicht verfuegbar ist.
...
Das Fenster darf die konfigurierte maximale Dauer von 15, 30, 60 oder 120 Minuten nicht ueberschreiten.

Die Speicherwahl `Automatisch`, `RAM` oder `Festplatte` veraendert die sichtbare Timeline-Bedienung nicht.
...
Das Auto-Next-Panel wird nur angezeigt, wenn eine naechste Episode vorhanden ist.
...
Der Zahlenwert wird sekundenweise aktualisiert. Hauptbutton und `Zurueck` erscheinen in beiden Varianten zeitgleich nebeneinander; der Fokus liegt auf dem Hauptbutton. Einen Button `Abbrechen` gibt es nicht.

OK auf dem Hauptbutton startet die naechste Episode sofort. Ohne Eingabe startet sie bei aktivem Auto-Next beim Ablauf des Countdowns am tatsaechlichen Episodenende. Bei deaktiviertem Auto-Next erfolgt ohne OK kein Start.

OK auf `Zurueck` oder die Zurueck-Taste beendet den Ablauf, verwirft einen laufenden Countdown und fuehrt zur Serien-Detailseite mit dem zuvor gewaehlten Staffel-/Episodenkontext zurueck.
...
Die feste Abschluss-Schwelle von 95 Prozent veraendert nur den gespeicherten Gesehen-/Fortsetzen-Status. Ihr Erreichen beendet die Wiedergabe nicht, blendet kein Auto-Next-Panel ein und startet keine naechste Episode.
...
- kehrt Vivicast zum passenden Film- oder Serienkontext zurueck
...
Pause, Vor- und Zurueckspulen werden nicht als separate Hauptbuttons benoetigt, weil die Timeline diese Bedienung uebernimmt.
...
- Zurueck schliesst das normale Player-Overlay, wenn es sichtbar ist.
- OK auf dem sichtbaren Button `Zurueck` oder die Zurueck-Taste beendet den Auto-Next-Ablauf und kehrt mit wiederhergestelltem Staffel-/Episodenkontext zur Serien-Detailseite zurueck.
- Zurueck verlaesst den Player, wenn kein Overlay oder Panel sichtbar ist.

## Rueckkehr nach Player

- Live-TV kehrt zum Sender im Live-TV Browser zurueck.
- Film kehrt zur Film-Detailseite zurueck.
- Serie kehrt zur Episode oder Serien-Detailseite zurueck.
- Externe Film- oder Episodenwiedergabe kehrt zum Film- oder Serienkontext zurueck und zeigt einen Hinweis zum nicht ermittelbaren Fortschritt.
...
Catch-Up verhaelt sich wie VOD mit EPG-Kontext.

Catch-Up wird intern abgespielt und erzeugt keinen VOD-Fortschritt.

Externe Player sind nur fuer Filme und Episoden vorgesehen, nicht fuer Live-TV oder Catch-Up.
