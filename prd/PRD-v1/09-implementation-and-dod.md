...
Diese Datei definiert fachliche Akzeptanzkriterien und Release-Definition-of-Done für Vivicast.

Sie enthält bewusst keinen festen Implementierungsplan und keine Codex-Vorgaben.
...
- eine Änderung gilt ab dem nächsten regulaeren App-Start und navigiert die laufende Sitzung nicht sofort um
- App-Autostart verwendet den konfigurierten Startbereich
- explizite Deep Links und Android-TV-Ziele haben Vorrang; Rueckkehr aus dem Hintergrund behält den aktuellen Kontext
...
- Home-Fortsetzen enthält nur nicht abgeschlossene Filme und Episoden mit fortsetzbarem Fortschritt
...
- leeres Home zeigt Hinweis plus Aktion `Wiedergabeliste hinzufügen`
...
- doppelte Zurück-Bestätigung beendet aus der Top Navigation
...
- frisches Öffnen fokussiert die erste Kategorie des ersten Providers
...
- Layout nutzt linke Kategorien, rechtes Poster Grid und Detailbereich darüber
...
- Favoriten können verwaltet werden
- Trailer-Verhalten über YouTube funktioniert
- fehlende Poster werden per Fallback behandelt
- Detailinformationen werden angezeigt, soweit vorhanden
- Zurück geht Detailseite -> Grid -> Kategorien -> Top Navigation
...
- Auto-Next Countdown ist standardmaessig 10 Sekunden; 5, 10, 15 und 30 Sekunden sind möglich
- Countdown-Auswahl bleibt bei deaktiviertem Auto-Next sichtbar, ist aber nicht bedienbar
- bei deaktiviertem Auto-Next erscheint `Nächste Folge abspielen` erst nach dem tatsaechlichen Episodenende und startet nur nach OK
- bei aktiviertem Auto-Next erscheint `Nächste Folge in X` X Sekunden vor dem tatsaechlichen Episodenende und aktualisiert X sekundenweise
- OK auf dem Auto-Next-Hauptbutton startet die nächste Episode sofort; ohne Eingabe startet sie beim Ablauf am Episodenende
- der sichtbare Button `Zurück` erscheint zeitgleich neben dem Hauptbutton; das Panel besitzt keinen Button `Abbrechen`
- OK auf `Zurück` oder die Zurück-Taste verwirft einen laufenden Countdown und fuehrt zur Serien-Detailseite mit wiederhergestelltem Staffel-/Episodenkontext
...
- bei externer Episodenwiedergabe erscheinen kein Auto-Next-Panel und kein automatischer Wechsel zur nächsten Episode
- externe Episodenwiedergabe erzeugt oder aktualisiert keinen automatischen Fortschritt und setzt keinen Abschlussstatus
- Zurück geht Player -> Serienkontext -> Detailseite -> Grid -> Kategorien -> Top Navigation

## Suche

Muss funktionieren:

- frisches Öffnen fokussiert Suchfeld
...
- Sprachsuche startet nur über explizite Mikrofon-Aktion
...
- lokale Suche über Room FTS4
- Debounce von 300 ms
- leere Suche fuehrt keine Ergebnisabfrage aus
- ein normalisiertes Zeichen sucht nur Titel- oder Name-Prefixe für Kanäle, Filme und Serien
...
- einzelne Suchverlaufseintraege löschen
...
- frisches Öffnen fokussiert Allgemein
- Gruppen sind Allgemein, Wiedergabelisten, EPG, Optik, Wiedergabe, Kindersicherung, Speicher & Verlauf, Backup und Über die App
...
- HTTP User Agent gilt appweit für HTTP-Anfragen, sofern technisch anwendbar
- HTTP User Agent ist nicht pro Wiedergabeliste oder Provider konfigurierbar
- Wiedergabeliste hinzufügen startet mit Name als Pflichtfeld
...
- M3U zeigt keine Importauswahl für Live-TV, Filme oder Serien
...
- Xtream Codes bietet Importoptionen für Live-TV, Filme und Serien
...
- Bearbeiten erlaubt Zugangsdaten oder M3U-Daten zu ändern
- bei geänderten Zugangsdaten oder URLs wird vor Speichern erneut getestet
...
- Provider-Sender erhalten EPG über Mapping und Provider-EPG-Priorität
- manuelle EPG-Zuordnung gewinnt immer vor automatischer Zuordnung
- Logo-Priorität pro Provider kann gesetzt werden
- Gruppen können angezeigt, ausgeblendet und sortiert werden
- Update-Optionen pro Provider sind konfigurierbar
- Ablaufdatum und maximale Verbindungen werden angezeigt, falls vorhanden
- Provider löschen nutzt einfache Bestätigung
- wenn Einstellungsschutz aktiv ist, wird beim Löschen zusätzlich PIN abgefragt
...
- PIN-Eingaben bestätigen nach vier Ziffern nicht automatisch; die sichtbare Aktion bleibt erforderlich
...
- erfolgreiche PIN-Eingabe gibt nur den betroffenen Schutzbereich für die aktuelle App-Sitzung frei
...
- `Automatisch nächste Folge` ist ein Toggle unter Wiedergabe und standardmaessig Aus
- `Countdown nächste Folge` bietet 5, 10, 15 und 30 Sekunden; Standard sind 10 Sekunden
...
- der externe Player ist nur eine Wiedergabeübergabe und schreibt in v1 keinen automatischen VOD-Fortschritt zurück
- `Über die App` enthält den Bereich `Diagnose und Support`
...
- Logdatei-Inhalte können nicht in die Zwischenablage kopiert werden
...
- die Logdatei enthält nur die freigegebenen technischen Ereigniskategorien
- die Metadatendatei enthält App-/Build-, Geräte-, Datenbank-, Sprach-, Zeitzonen-, Zeitraum-, Sitzungs-, Grenzwert- und Trunkierungsdaten
- pro App-Prozessstart bei aktiver Protokollierung entsteht eine neue logische Diagnosesitzung
- offene Sitzungen werden beim nächsten Prozessstart systemgestuetzt oder anhand von `lastRecordedAt` abgeschlossen
...
- sitzungsinterne Rotation entfernt das aelteste geschlossene Segment; globale Größenbereinigung entfernt zuerst die aeltesten abgeschlossenen Sitzungen
...
- Speicher & Verlauf zeigt lokale Cache-Wartung mit aktueller Größe des Medien-Caches und der Aktion `Cache leeren`
...
- Speicher & Verlauf bietet Loeschaktionen für Live-TV-Verlauf, Filmverlauf mit Film-Wiedergabefortschritt, Serienverlauf mit Episoden-Wiedergabefortschritt, Suchverlauf und gesamten Verlauf
- Verlaufslimits sind in v1 nicht frei konfigurierbar; der Suchverlauf bleibt fest auf maximal 20 Eintraege begrenzt
- wenn aktuell lokal Einstellungsschutz aktiv ist oder die lokale Schutzkonfiguration eine PIN für Backup/Restore verlangt, fragt Restore vor dem Einspielen die aktuell lokale PIN ab
- verschlüsselte Vollbackups nutzen ein versioniertes AES-GCM-Schutzformat mit KDF-Metadaten, Salt, Nonce, Ciphertext und Authentifizierungstag
...
- PIN-Pruefwerte, aktive PIN-Freigaben und Kindersicherung-Schutzflags aus Backup-Dateien werden nach Restore nicht übernommen
...
- Xtream Codes
- XMLTV
- toleranter Teilimport für M3U, Xtream Codes und XMLTV
...
- Serienwiedergabe
- Catch-Up Wiedergabe
- externer Player nur für Filme und Episoden, nicht für Live-TV oder Catch-Up
- PlaybackRequest enthält Medientyp, Provider, stabile Medienreferenz, Herkunft, Rueckkehrziel, Startposition und EPG-/Timeshift-Kontext
...
- Audio-Auswahl
- Untertitel-Auswahl
- globale Audio-/Untertitel-Defaults werden beim Streamstart angewendet; manuelle Player-Auswahl gilt nur für die aktuelle Wiedergabe
- Bildformat-Auswahl
- Retry bei Startfehlern
- Reconnect bei Streamabbruch
- nur eine aktive Wiedergabe gleichzeitig
- höchstens ein aktiver Timeshift-Puffer
...
- Timeshift gilt nur für Live-TV; Speicher- oder Ressourcenfehler lassen Live-TV ohne Timeshift weiterlaufen
...
- aktuelle Live-Sendungen werden über Live-TV oder Timeshift und nicht als Catch-Up gestartet
...
- Hauptbutton und `Zurück` erscheinen im Folgepanel zeitgleich; OK auf `Zurück` oder die Zurück-Taste fuehrt mit wiederhergestelltem Staffel-/Episodenkontext zur Serien-Detailseite
- Rueckkehr zu Sender, Film-Detailseite oder Serienkontext gemaess Herkunft
- der interne Player ist die einzige Quelle für automatische VOD-Position, Dauer, Fortschritt, Medienende, Abschlussstatus und Auto-Next
...
- nach Rueckkehr aus einem externen Film- oder Episodenplayer bleibt vorhandener Fortschritt unverändert und Vivicast zeigt einen Hinweis, dass der Fortschritt nicht automatisch ermittelt werden konnte
...
- Deep Links
- Watch Next für Filme und Serienepisoden
- Android-TV-Systemsuche enthält Live-TV, Filme und Serien, aber keine EPG-Treffer, keine Episoden als eigene Treffer und kein Catch-Up
...
- Deep Links, Android-TV-Suchergebnisse und Watch Next pruefen beim Öffnen aktuelle Providerverfuegbarkeit, Zugangsdatenstatus und Kindersicherung
...
- Watch Next wird nur für Filme und Serienepisoden aus internem Playback Progress und manuellen Gesehen-/Ungesehen-Aktionen gepflegt
- externe Player erzeugen keine Watch-Next-Updates
- Provider-Deaktivierung, Provider-Loeschung, Restore, Migration und Schutzregel-Änderung bereinigen Android-TV-Systemsuche und Watch Next
...
- `startDestination` erlaubt ausschliesslich `HOME`, `LIVE_TV`, `MOVIES` und `SERIES`; ungültige Werte fallen auf `HOME` zurück
...
- EPG-App-Start- und Playlist-Änderungs-Refresh werden als `epgRefreshOnAppStartEnabled = true` und `epgRefreshOnPlaylistChangeEnabled = true` in DataStore gespeichert
- `localLogoFolderUri` ist standardmaessig `null` und verweist bei gesetztem Wert auf eine persistierbare Android-SAF-URI
- der Exportdialog enthält `Backup-Typ` nur als transientes Dialogfeld mit Startwert `STANDARD`; es gibt keine eigene Settings-Zeile und keinen dauerhaft gespeicherten Wert
...
- Auto-Next wird als `autoNextEpisodeEnabled = false` und `autoNextEpisodeCountdownSeconds = 10` gespeichert; als Countdown sind nur 5, 10, 15 und 30 zulässig
...
- Größe des Medien-Caches, Cache-Rotation und frei konfigurierbare Verlaufslimits werden nicht in DataStore gespeichert
- Medien-Cache-Dateien sind nicht Teil des Standard-Backups
- PIN-Pruefwerte, aktive PIN-Freigaben und Kindersicherung-Schutzflags aus Backups werden nicht in lokale Sicherheitszustaende übernommen
...
- normale nicht geheime Quellenadressen dürfen als Konfiguration gespeichert werden
...
- HTTP für nutzerdefinierte IPTV-, EPG- und Stream-Quellen bleibt erlaubt, aber sichtbar unsicher und zentral policygebunden
- TLS-Zertifikatsfehler werden nicht bypassed
...
- optional verschlüsselte Backups
- verschlüsselte Vollbackups schützen Geheimnisse mit Passphrase und versioniertem AES-GCM-Container
- Restore verwendet für PIN-Autorisierung ausschliesslich die aktuell lokale PIN und nicht die Backup-Datei
...
- manuelle Android-TV-QA ist nur für Fokus, D-Pad, Back-Pfade, visuelle Skalierung, echte TV-Geräte und Systemoberflaechen zulässig
...
- Refresh-Fehler vor Commit verändern keine produktiven Inhalte
- Commit-Fehler rollen die betroffene Provider- oder EPG-Transaktion vollstaendig zurück
- aktive Streams werden durch Refreshes nicht unterbrochen

## Performance

Messbare Zielbudgets für Import, Suche, Datenbank, EPG und Speicherverbrauch liegen verbindlich in `prd/PRD-v1/13-test-strategy.md`.

Fluessige Bedienung bei Zielgrößen:
...
- Performancebudgets sind auf dem dokumentierten Referenzgerät oder Referenzprofil gemessen
...
- Zurück-Verhalten ist konsistent
...
- Teilfehler dürfen valide Eintraege übernehmen, aber Loeschungen nur aus autoritativen Teilbereichen ableiten
...
- Verlauf und Playback Progress bleiben korrekt und können nach Restore bis zum Provider-Refresh pending sein
...
- Diagnosedaten überschreiten weder 20 MiB Gesamtgröße noch 2 MiB je Segment noch drei Segmente pro Sitzung
- größenbedingte Kuerzungen und Entfernungen werden im Diagnoseexport nachvollziehbar ausgewiesen
...
- das Diagnose-ZIP enthält die verbindlichen Log- und Metadatendateien
...
- Speicher-&-Verlauf-Verwaltung funktioniert
- Speicher & Verlauf zeigt die aktuelle Größe des Medien-Caches und bietet `Cache leeren`
- `Cache leeren` entfernt nur Medien-Cache-Dateien und laesst fachliche lokale Daten unverändert
...
- Fehlerprotokolle sind für Support exportierbar, ohne ihren Inhalt in der App anzuzeigen
